...
export default class Color {
	...
	static hue (idx) {
		...
		let hue = ridx / Math.pow(2, bitcount);
	...
	static getColor (idx) {
		if (typeof idx === 'string') {
			this.addSource(idx);
			idx = this.sources.indexOf(idx);
		}

		return this.fromHSL(Math.round(this.hue(idx) * 360), 100, 50);
	}


	static addSource (userId) {
		let {sources} = this;
		if (!sources) {
			sources = [];
		...
		if (userId && !sources.includes(userId)) {
			sources.push(userId);

			//assume the first source is the app user, keep it at index 0
			let appUser = sources.shift();

			sources.sort();

			sources.unshift(appUser);
		}
	}
}
