...
function copyDimensions (from, to) {
	Object.apply(to, getDimensions(from));
}
...
function scaleImageDown(image, maxW, maxH, transform) {
	...
	if (pixels < maxPixels) {
		...
		return {
			width, height, src: (image.src || image.toDataURL('image/png'))
		};
	}
...
function buildCanvasFromImage (img) {
	...
	if (height > width || (height * scale) > (1 / data.viewportRatio)) {
		scale = (1 / data.viewportRatio) / height;
		wbCY = (scale * height) / 2;
		wbCX = 0.5;
	}
...
function correctOrientation (image, transform) {
	...
	let flippedDimensions = (r=> r !== 0 && r !== Math.PI)(transform.getRotation());
...
export function createFromImage (img) {
	...
	function enforceMaxSize(input, output, transform) {
		...
		try {
			...
			proxy.onload = ()=> output.src = scaleImageDown(proxy, X, Y, transform).src;
